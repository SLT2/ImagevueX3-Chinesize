    ...
    public function create_user_panel($user_id)
    {
        ...
        if( $this->db_use ) {
            ...
            if( in_array( "edit_settings", $this->user_permissions ) ) {
                $settings_menu = '<li id="setting"><a href="javascript:;">Settings</a></li>';
            }
        }
        else {
        ...
        {
            $this->user_menu = '<li id="fileManager"><a href="javascript:;">'.language_filter("File Manager", true).'</a></li>
                                '.$settings_menu.'
                                <li id="tickets" '.$display_ticket.'><a href="javascript:;" onclick="show_what = \'all\'; ticket_page = 1;">'. language_filter("Tickets", true).'</a></li>
                                <li id="editProfile"><a href="javascript:;">'.language_filter("Edit Profile", true).'</a></li>
        ...
        {
            $this->user_can .= '<button class="btn btn-default btn-group-filemanager" onclick="$(\'#container_id_tree3\').html(\'<div class=\\\'alert alert-info\\\'><center><b>'.language_filter("Choose your target directory.", true, true).'</b></center></div>\'); if(selected == \'\' || selected == null) alert(\''.language_filter("Please select files and folders", true, true).'\'); else $(\'#copySelected\').modal(\'show\');">'.language_filter("Copy", true).'</button>';
        ...
        {
            $this->user_can .= "<button class=\"btn btn-default btn-group-filemanager\" onclick=\"if(selected == '' || selected == null) alert('".language_filter("Please select files and folders", true, true)."'); else check_selected_files_folders();\">".language_filter("Remove", true)."</button>";
        ...
        {
            $this->user_can .= "<button class=\"btn btn-default btn-group-filemanager\" onclick=\"if(selected == '' || selected == null) alert('".language_filter("Please select files and folders", true, true)."'); else $('#newzipFile').modal('show');\">".language_filter("Zip", true)."</button>";
        }
