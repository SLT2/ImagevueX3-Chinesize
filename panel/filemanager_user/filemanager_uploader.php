...
?>
        ...
        <form id="fileupload" action="upload.php" method="POST" enctype="multipart/form-data">
            <input type="hidden" name="uploadDir" value="<?php echo $_POST["upload_dir"];?>">
            ...
            <div class="fileupload-buttonbar">
                ...
                <div class=fileupload-buttons>
	                ...
	                <?php /*<span class="btn btn-success btn-lg fileinput-button">
	                    <i class="glyphicon glyphicon-plus"></i>
	                    <span><?php language_filter( "Add_Files" );?></span>
	                    <input type="file" name="datafile" multiple>
	                </span> */ ?>
                ...
                <tr class="template-download fade">
                        ...
                        <span class="preview">
                            {% if (file.thumbnailUrl) { %}
                                <?php /*<a href="{%=file.url%}" title="{%=file.name%}" download="{%=file.name%}" data-gallery><img src="{%=file.thumbnailUrl.replace('/content/','/render/w'+thumbsize+'-c1:1-q90/')%}" class="jquery_uploader_thumbnail gaga"></a> */ ?>
                                <a href="{%=file.thumbnailUrl.replace('//content/','/content/')%}" class="popup" data-width="{%=file.width%}" data-height="{%=file.height%}" data-name="{%=file.name%}" data-filesize="{%=o.formatFileSize(file.size)%}" rel="up1">
                    ...
                    <td class=td-data>
                        <div class="filename"><a href="{%=file.thumbnailUrl.replace('//content/','/content/')%}" class="popup" data-width="{%=file.width%}" data-height="{%=file.height%}" data-name="{%=file.name%}" data-filesize="{%=o.formatFileSize(file.size)%}" rel="up2">{%=file.name%}</a></div>
                        {% if (file.error) { %}
                            <div><span class="label label-danger"><?php language_filter( "Error" );?></span> {%=file.error%}</div>
                        {% } %}
                        <div class="size">{%=o.formatFileSize(file.size)%} | {%=file.width%} x {%=file.height%}</div>
                    </td>
        ...
        <script>
            $(function () {
            	x3Uploader({
            		...
            		"orientation": <?php echo ($x3_config["back"]["panel"]["upload_resize"]["orientation"] ? 'true' : 'false'); ?>
            	});
            });
